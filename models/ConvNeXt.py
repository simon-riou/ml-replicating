...
class Block(nn.Module):
    def __init__(self, C, p=0.2, downsample=False, layer_scale_init_value=1e-6):
...
class ConvNeXt(nn.Module):
    def __init__(self, in_channels, num_classes, C, B=[3, 3, 9, 3]):
        super(ConvNeXt, self).__init__()

        self.stem = nn.Conv2d(in_channels, C, 4, 4, 0)

        self.ln_res2 = nn.LayerNorm(C)
        self.res2 = nn.ModuleList([Block(C) for _ in range(B[0])])

        self.ln_res3 = nn.LayerNorm(C)
        self.res3 = nn.ModuleList([Block(C, downsample=True)] + [Block(C * 2) for _ in range(B[1] - 1)])

        self.ln_res4 = nn.LayerNorm(C*2)
        self.res4 = nn.ModuleList([Block(C * 2, downsample=True)] + [Block(C * 4) for _ in range(B[2] - 1)])

        self.ln_res5 = nn.LayerNorm(C*4)
        self.res5 = nn.ModuleList([Block(C * 4, downsample=True)] + [Block(C * 8) for _ in range(B[3] - 1)])
...
def ConvNeXtT(in_channels=3, num_classes=1000):
    """ConvNeXtT model"""
    return ConvNeXt(in_channels, num_classes, 96, [3, 3, 9, 3])


def ConvNeXtS(in_channels=3, num_classes=1000):
    """ConvNeXtS model"""
    return ConvNeXt(in_channels, num_classes, 96, [3, 3, 27, 3])


def ConvNeXtB(in_channels=3, num_classes=1000):
    """ConvNeXtB model"""
    return ConvNeXt(in_channels, num_classes, 128, [3, 3, 27, 3])


def ConvNeXtL(in_channels=3, num_classes=1000):
    """ConvNeXtL model"""
    return ConvNeXt(in_channels, num_classes, 192, [3, 3, 27, 3])


def ConvNeXtXL(in_channels=3, num_classes=1000):
    """ConvNeXtXL model"""
    return ConvNeXt(in_channels, num_classes, 256, [3, 3, 27, 3])
